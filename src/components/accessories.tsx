...
type Category = 'Accessories / Coils';
...
const AccessoriesCarousel = () => {
    const [activeTab, setActiveTab] = useState<Category>('Accessories / Coils');
    const categories: Category[] = ['Accessories / Coils'];
    ...
    const filteredProducts = useMemo(() => {
        return products.filter((p) => {
            ...
            return cat.includes('accessories') || cat.includes('coil') || name.includes('accessories') || name.includes('coil') || cat.includes('hardware');
        });
    }, [products]);
            ...
            <div className="relative px-2 md:px-8">
                {loading ? (
                    <div className="py-20 flex flex-col justify-center items-center gap-3 text-gray-500">
                        ...
                        <span className="text-xs font-bold tracking-widest uppercase">Loading Accessories...</span>
                    </div>
                ) : filteredProducts.length === 0 ? (
                    <div className="py-20 flex justify-center items-center text-gray-500 text-sm font-bold tracking-widest uppercase">
                        No products found for Accessories.
                    </div>
                ) : (
                    <Swiper
                        modules={[Navigation, Pagination]}
                        spaceBetween={10}
                        slidesPerView={1} // Set to 1 for mobile view
                        navigation={{
                        ...
                        breakpoints={{
                            // Maintains single card up to 640px wide devices
                    ...
                    >
                        {filteredProducts.map((product) => {
                            const imageUrl = product.images?.[0] || product.productImage || product.mainImage || product.variants?.[0]?.image || '/cards/card1.webp';
                                ...
                                <SwiperSlide key={product._id}>
                                    <div className="bg-white p-2 border border-transparent md:hover:border-gray-100 md:hover:shadow-lg transition-all duration-300 group/card relative rounded-sm h-full flex flex-col">
                                        ...
                                        <div className="mt-2 text-center pb-2">
                                            <p className="text-[8px] md:text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">{product.brand || 'ACCESSORY'}</p>
