...
const BRAND_DATA = {
    hardware: [
        "SMOKTech", "Uwell", "VOOPOO", "Vandy Vape", "Geek Vape",
        "Vaporesso", "YoCan", "Lookah", "FreeMaX", "CCELL"
    ],
    liquids: [
        "Pod Juice", "Four Seasons", "Monster Vape Labs", "7 Daze",
        "Juice Head", "SKWEZED", "Fifty Bar", "Ruthless Vapor"
    ]
};
...
const Navbar = () => {
    ...
    const currentProducts = MEGAMENU_CONTENT[activeMenu || 'brands'] || MEGAMENU_CONTENT['brands'];
        ...
        >
            ...
            <div className="mx-auto flex max-w-7xl items-center justify-between px-4 h-20 lg:h-24 lg:px-8">
                ...
                <div className="flex-shrink-0 flex items-center lg:static absolute left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0">
                    <Link href="/" className="group relative block h-12 w-12 md:h-24 md:w-20 transition-transform duration-300 hover:scale-105">
                        <Image
                            src="/logo.jfif"
                            alt="Logo"
                            fill
                            ...
                            >
                                ...
                                >
                                    <span className="relative pb-1">
                                        {link.label}
                                        <span className={`absolute bottom-0 left-0 h-[2px] bg-gray-400 transition-all duration-300 ease-in-out ${isActive || isHovered ? 'w-full opacity-100' : 'w-0 opacity-0'}`} />
                                    </span>
                                    {link.id !== 'new' && (
                                        <ChevronDown
            ...
            <div className={`fixed top-0 left-0 h-full w-[80%] max-w-sm bg-black border-r border-zinc-800 z-[120] transform transition-transform duration-300 ease-in-out lg:hidden ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex items-center justify-between p-6 border-b border-zinc-900">
                    <span className="text-sm font-black tracking-widest text-zinc-500 uppercase">Menu</span>
                ...
                <div className="overflow-y-auto h-[calc(100vh-80px)]">
                    ...
                    <div className="mt-8 px-6 pt-8 border-t border-zinc-900">
                        <p className="text-[10px] font-black text-zinc-600 tracking-[0.3em] uppercase mb-4">Quick Links</p>
                        <div className="flex flex-col space-y-4">
                            <Link href="/dashboard" className="text-sm text-zinc-400">Admin Dashboard</Link>
            ...
            >
                <div className="mx-auto max-w-7xl px-8 py-14 grid grid-cols-12 gap-12">
                    ...
                    <div className="col-span-3">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-zinc-600 mb-8 pb-3 border-b border-zinc-800">HARDWARE</h3>
                        <div className="flex flex-col space-y-4">
                            {BRAND_DATA.hardware.map((item) => (
                                <Link key={item} href="/collection" className="text-sm font-bold text-zinc-400 hover:text-white hover:translate-x-2 transition-all duration-200">{item}</Link>
                            ))}
                        </div>
                    </div>

                    <div className="col-span-3">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.25em] text-zinc-600 mb-8 pb-3 border-b border-zinc-800">E-LIQUIDS</h3>
                        <div className="flex flex-col space-y-4">
                    ...
                    <div className="col-span-6 grid grid-cols-2 gap-8">
                        {currentProducts.map((product, idx) => (
                            <Link key={idx} href="/collection" className="group cursor-pointer block">
                                ...
                                <div className="mt-6 flex flex-col items-center text-center">
                                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 mb-2">Featured Collection</p>
